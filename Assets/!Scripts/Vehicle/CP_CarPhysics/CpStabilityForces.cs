...
namespace _Scripts.Vehicle.CP_CarPhysics
{
    ...
    public class CpStabilityForces : MonoBehaviour
    {
        ...
        void ApplyLinearStabilityForces(Rigidbody rigidbody, Transform[] physicsWheelPoints, bool grounded, int numberOfGroundedWheels)
        {
            if (linearStabilityForce > 0 && grounded && numberOfGroundedWheels < 3)
            {
                var downwardForce = linearStabilityForce * Vector3.down * Time.fixedDeltaTime;
        ...
        void ApplyAngularStabilityForces(Rigidbody rigidbody, Vector3 averageColliderSurfaceNormal, bool grounded)
        {
            if (averageColliderSurfaceNormal != Vector3.zero && !grounded)
            {
                ...
                var angle = Vector3.SignedAngle(rigidbody.transform.up, averageColliderSurfaceNormal, rigidbody.transform.forward);

                //Angular stability only uses roll - Using multiple axis becomes unpredictable 
                var torqueAmount = Mathf.Sign(angle) * rigidbody.transform.forward * angularStabilityForce * Time.fixedDeltaTime;
