using System;
using GitAmendKartSample.Scripts.Utils;
...
namespace GitAmendKartSample.Scripts {
    public class AIInput : MonoBehaviour, IDrive {
        ...
        void Start() {
            if (circuit == null) {
                throw new ArgumentNullException($"AIInput requires a circuit to be set.");
            }
            if (driverData == null) {
                throw new ArgumentNullException($"AIInput requires driver data to be set.");
            }
